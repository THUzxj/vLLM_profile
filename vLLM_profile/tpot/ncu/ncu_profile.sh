...
for BATCH_SIZE in ${BATCH_SIZES[@]}; do
    for KV_LEN in ${KV_LENS[@]}; do
        OUTPUT_FILE="${OUTPUT_DIR}/ncu_report_flash_attn_bs${BATCH_SIZE}_kv${KV_LEN}_model${MODEL_NAME}"
        ncu \
            --set full \
            ...
            --kernel-regex "(ampere_.*|act_and_mul_kernel|flash_fwd_splitkv_kernel|Kernel2)" \
