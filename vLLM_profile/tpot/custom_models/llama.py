...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        times["qkv_projection_and_rope"] = end - start

        start = timing_function()
        attn_output = self.attn(q, k, v)
        torch.cuda.synchronize()
        end = timing_function()
        times["attention_forward"] = end - start

        start = timing_function()
        output, _ = self.o_proj(attn_output)
        torch.cuda.synchronize()
        end = timing_function()
        times["output_projection"] = end - start
        return output, times
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        torch.cuda.synchronize()
        start = timing_function()
        hidden_states, attn_times = self.self_attn(positions=positions, hidden_states=hidden_states)
        torch.cuda.synchronize()
        end = timing_function()
        ...
        times["attention_details"] = attn_times
...
class LlamaModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        # get env
        self.profile_component_output_dir = os.getenv("PROFILE_COMPONENT_OUTPUT_DIR", None)
        if self.profile_component_output_dir is not None:
            self.output_dir = self.profile_component_output_dir
        else:
            self.profile_component_bs = os.getenv("PROFILE_COMPONENT_BS", None)
            if self.profile_component_bs is not None:
                self.profile_component_bs = int(self.profile_component_bs)
            else:
                self.profile_component_bs = "unknown"

            self.profile_component_in = os.getenv("PROFILE_COMPONENT_IN", None)
            if self.profile_component_in is not None:
                self.profile_component_in = int(self.profile_component_in)
            else:
                self.profile_component_in = "unknown"

            self.profile_component_model = os.getenv("PROFILE_COMPONENT_MODEL", "llama-7b")

            self.output_dir = f"component_times_{self.profile_component_model}/{self.profile_component_model}_in{self.profile_component_in}_bs{self.profile_component_bs}/"
    ...
    def forward(
        ...
        for idx, layer in enumerate(
            ...
            start = timing_function()
            hidden_states, residual, layer_times = layer(positions, hidden_states, residual)
            torch.cuda.synchronize()
            end = timing_function()
        ...
        if input_ids is not None:
            input_shape_str = str(input_ids.shape)
        elif inputs_embeds is not None:
            input_shape_str = str(inputs_embeds.shape)
        else:
            input_shape_str = "unknown"
        log_file = f"{self.output_dir}/count_{self.count}_promptlenshape_{input_shape_str}_time{timing_function()}.json"
        with open(log_file, "w") as f:
            import json
            json.dump({"model_time": model_time, "layer_times": all_layer_times}, f, indent=4)
