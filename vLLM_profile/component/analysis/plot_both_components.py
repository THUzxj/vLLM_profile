...
def plot_batch_size_comparison(
    ...
    # Use only the intersection of batch_size values from both components
    common_batch_sizes = sorted(
        set(attn_df["batch_size"].unique()) & set(mlp_df["batch_size"].unique())
    )
    if not common_batch_sizes:
        raise ValueError("No common batch_size values between attention and MLP data.")

    attn_df = attn_df[attn_df["batch_size"].isin(common_batch_sizes)]
    mlp_df = mlp_df[mlp_df["batch_size"].isin(common_batch_sizes)]

    plt.figure(figsize=(12, 8))

    # Plot MLP (seq_len=1, decode phase) on common batch_size axis
    ...
    # Plot Attention Layer for different kv_len values on common batch_size axis
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to attention layer results CSV file. "
            "Defaults to <output-dir>/benchmark_flash_attn_results.csv"
        ),
    )
    parser.add_argument(
        "--mlp-csv",
        ...
        default=None,
        help=(
            "Path to MLP results CSV file. "
            "Defaults to <output-dir>/benchmark_qwen3_mlp_results.csv"
        ),
    )
    parser.add_argument(
        "--output-dir",
    ...
    # Set defaults that depend on output_dir if not explicitly provided
    if args.attention_csv is None:
        args.attention_csv = os.path.join(
            args.output_dir, "benchmark_flash_attn_results.csv"
        )
    if args.mlp_csv is None:
        args.mlp_csv = os.path.join(
            args.output_dir, "benchmark_qwen3_mlp_results.csv"
        )
