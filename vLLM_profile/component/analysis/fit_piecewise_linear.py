...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='分段线性函数拟合脚本')
    ...
    # 读取 CSV 文件
    csv_path = os.path.join(result_dir, 'benchmark_qwen3_mlp_results.csv')
    ...
    # 拟合分段线性函数
    print("\n开始拟合分段线性函数...")
    breakpoint, a1, b1, a2, b2, r2, mse = fit_piecewise_linear(
        batch_size, mean_time_ms)
    ...
    # 原始数据点
    plt.scatter(batch_size, mean_time_ms, color='blue',
                s=100, alpha=0.7, label='原始数据', zorder=3)

    # 拟合曲线
    plt.plot(x_fit, y_fit, 'r-', linewidth=2, label='分段线性拟合', zorder=2)

    # 标记分段点
    y_breakpoint = piecewise_linear(
        np.array([breakpoint]), breakpoint, a1, b1, a2, b2)[0]
    ...
    plt.plot(x1_fit, y1_fit, 'orange', linestyle=':',
             linewidth=1.5, alpha=0.7, label='第一段', zorder=1)
    plt.plot(x2_fit, y2_fit, 'purple', linestyle=':',
             linewidth=1.5, alpha=0.7, label='第二段', zorder=1)
    ...
    output_path = os.path.join(
        result_dir, 'benchmark_qwen3_mlp_piecewise_linear_fit.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\n图片已保存到: {output_path}")

    # 保存拟合参数到文件
    params_path = os.path.join(
        result_dir, 'benchmark_qwen3_mlp_piecewise_linear_params.txt')
