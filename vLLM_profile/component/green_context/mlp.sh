...
for model_size in "4B" "32B"; do
    OUTPUT_DIR="green_context_profile_results_3/${model_size}_num_processes_2_mode_${MODE}_rep2"
    mkdir -p ${OUTPUT_DIR}
    ...
    python green_context/benchmark_qwen3_mlp_green_ctx.py --batch-sizes 8192 \
      --seq-lens 1 --output-dir ${OUTPUT_DIR} \
      --model-size ${model_size} --num-processes 2 --min-sm-count 50 --num-groups 2
