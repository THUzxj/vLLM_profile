...
def analyze_layer_times(json_files):
    """
    分析多个JSON文件中所有layer的时间组件统计信息，合并统计
    """
    # 初始化数据集合
    total_layer_times = []
    ...
    for json_file in json_files:
        ...
        if 'model_time' in data:
            model_times.append(data['model_time'])
    ...
    if model_times:
        stats_list.insert(0, calculate_stats(model_times, 'model_time'))
...
def get_max_count_files(folder_path, num_files=1):
    """
    获取文件夹中count_后面数字最大的若干个文件
    """
    ...
    # 按count数字排序，取最大的num_files个
    sorted_files = sorted(json_files, key=extract_count, reverse=True)
...
# 解析命令行参数
parser = argparse.ArgumentParser(description='分析layer时间组件统计信息')
parser.add_argument('folder', nargs='?', default='./component_times/', 
                    help='component_times文件夹路径 (默认: ./component_times/)')
parser.add_argument('-n', '--num-files', type=int, default=1,
                    help='合并count数字最大的N个JSON文件 (默认: 1)')

args = parser.parse_args()

# 获取component_times文件夹
component_times_folder = Path(args.folder)
num_files = args.num_files

if num_files < 1:
    print("错误: num_files 必须大于等于 1")
    sys.exit(1)
...
for subfolder in sorted(component_times_folder.iterdir()):
    ...
    # 获取count数最大的若干个JSON文件
    json_files = get_max_count_files(subfolder, num_files)
    ...
    try:
        # 分析数据（合并多个文件）
        df = analyze_layer_times(json_files)
        ...
        file_names = ', '.join([f.name for f in json_files])
        print(f"✓ {subfolder.name} / [{file_names}]: 已保存到 {output_csv.name} (合并了 {len(json_files)} 个文件)")
