...
class DeepseekV2MoE(nn.Module):
    ...
    def get_expert_statistics(self, router_logits, times):
        ...
        times["num_expert_requests"] = topk_indices.shape[0] * topk_indices.shape[1]

        if self.profile_component_output_dir is not None:
            # save the topk_indices to a file
            topk_indices_file = os.path.join(self.profile_component_output_dir, f"topk_indices_{self.count}.pt")
...
class DeepseekV2Model(nn.Module):
    ...
    def __init__(
        ...
        print("Initializing customed DeepSeek model")
        ...
        if self.profile_component_output_dir is not None:
            ...
            print("Using Profile component output dir:", self.output_dir)
        else:
            ...
            self.profile_component_model = os.getenv(
                "PROFILE_COMPONENT_MODEL", "deepseek-v2"
            )
            self.output_dir = (
                f"component_times_{self.profile_component_model}/"
                f"{self.profile_component_model}_in{self.profile_component_in}_"
                f"bs{self.profile_component_bs}/"
            )
