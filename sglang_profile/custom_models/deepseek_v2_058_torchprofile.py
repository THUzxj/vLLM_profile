...
class DeepseekV2MLP(nn.Module):
    ...
    def forward(
        ...
        with record_function(f"component_range:mlp_gate_up_proj"):
            gate_up, _ = self.gate_up_proj(x)
        with record_function(f"component_range:mlp_act_fn"):
            x = self.act_fn(gate_up)
        with record_function(f"component_range:mlp_down_proj"):
            x, _ = self.down_proj(
                x,
...
class DeepseekV2MoE(nn.Module):
    ...
    def __init__(
        ...
        print("Initialized DeepseekV2MoE with torch profile record_functions")
    ...
    def forward_normal_dual_stream(
        ...
        with record_function(f"component_range:moe_shared_experts_layer{self.layer_id}"):
            shared_output = self._forward_shared_experts(
                hidden_states, gemm_output_zero_allocator
            )

        with torch.cuda.stream(self.alt_stream):
            with record_function(f"component_range:moe_gate_layer{self.layer_id}"):
                router_logits = self.gate(hidden_states, gemm_output_zero_allocator)
            with record_function(f"component_range:moe_topk_layer{self.layer_id}"):
                topk_output = self.topk(hidden_states, router_logits)
            with record_function(f"component_range:moe_experts_layer{self.layer_id}"):
                final_hidden_states = self.experts(hidden_states, topk_output)
            if not _is_cuda or isinstance(self.experts.quant_method, KTEPWrapperMethod):
                final_hidden_states *= self.routed_scaling_factor

        current_stream.wait_stream(self.alt_stream)
        with record_function(f"component_range:moe_combine_output_layer{self.layer_id}"):
    ...
    def op_dispatch_a(self, state):
        if self.ep_size > 1:
            with record_function(f"component_range:moe_dispatch_layer{self.layer_id}"):
    ...
    def op_experts(self, state):
        with record_function(f"component_range:moe_core_layer{self.layer_id}"):
    ...
    def op_combine_a(self, state):
        if self.ep_size > 1:
            with record_function(f"component_range:moe_combine_layer{self.layer_id}"):
...
class DeepseekV2AttentionMLA(nn.Module, DeepseekMHAForwardMixin):
    ...
    def op_prepare(self, state):
        with record_function(f"component_range:attention_prepare_layer{self.layer_id}"):
    ...
    def op_core(self, state):
        with record_function(f"component_range:attention_core_layer{self.layer_id}"):
...
class DeepseekV2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        with record_function(f"component_range:layer_communicator_prepare_layer{self.layer_id}"):
        ...
        with record_function(f"component_range:self_attention_layer{self.layer_id}"):
        ...
        with record_function(f"component_range:layer_communicator_prepare_mlp_layer{self.layer_id}"):
        ...
        with record_function(f"component_range:mlp_layer{self.layer_id}"):
        ...
        if not should_allreduce_fusion:
            with record_function(f"component_range:layer_communicator_postprocess_layer{self.layer_id}"):
    ...
    def forward(
        ...
        if self.pp_group.is_first_rank:
            if input_embeds is None:
                with record_function(f"component_range:embedding"):
        ...
        for i in range(normal_start_layer, normal_end_layer):
            ...
            with ctx:
                ...
                layer = self.layers[i]
                with record_function(f"component_range:layer_{i}"):
        ...
        if not self.pp_group.is_last_rank:
            ...
            if not forward_batch.forward_mode.is_idle():
                with record_function(f"component_range:final_norm"):
                    if residual is None:
...
class DeepseekV2ForCausalLM(nn.Module, DeepseekV2WeightLoaderMixin):
    ...
    def __init__(
        ...
        print("Initialized DeepseekV2ForCausalLM with torch profile record_functions")
    ...
    def forward(
        ...
        if self.pp_group.is_last_rank:
            with record_function(f"component_range:logits_processor"):
