...
class CustomedDeepEPMoE(DeepEPMoE):
    ...
    def forward_impl(self, hidden_states, topk_output):
        with nvtx.range(f"moe_dispatch_layer{self.layer_id}"):
            dispatch_output = self.dispatcher.dispatch(
                hidden_states=hidden_states, topk_output=topk_output
            )

        with nvtx.range(f"moe_core_layer{self.layer_id}"):
            combine_input = self.run_moe_core(dispatch_output)

        with nvtx.range(f"moe_combine_layer{self.layer_id}"):
...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def forward_normal(
        ...
        with nvtx.range(f"moe_gate_layer{self.layer_id}"):
            router_logits, _ = self.gate(hidden_states)

        with nvtx.range(f"moe_topk_layer{self.layer_id}"):
            topk_output = self.topk(hidden_states, router_logits)

        with nvtx.range(f"moe_experts_layer{self.layer_id}"):
...
class Qwen3MoeAttention(nn.Module):
    ...
    def forward(
        ...
        with nvtx.range(f"attention_prepare_layer{self.layer_id}"):
        ...
        with nvtx.range(f"attention_core_layer{self.layer_id}"):
...
class Qwen3MoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hidden_states.shape[0] != 0:
            with nvtx.range(f"self_attention_layer{self.layer_id}"):
        ...
        with nvtx.range(f"mlp_layer{self.layer_id}"):
