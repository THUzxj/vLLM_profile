...
input_ids=[[1, 450, 7483, 310, 3444, 338], [1, 450, 7483, 310, 278, 3303,
    13187, 290, 338], [1, 20628, 338, 263, 6575, 1460, 2462, 322, 306, 763]]
...
def prepare_synthetic_inputs_for_latency_test(
    ...
    if custom_inputs:
        # 截断或填充每个请求到input_len长度
        # custom_inputs是列表的列表，需要先转换为numpy数组
        padded_inputs = []
        for req in custom_inputs:
            req_len = len(req)
            if req_len == 0:
                # 如果req_len为0，用随机数替代
                padded_inputs.append(np.random.randint(
                    0, 10000, input_len, dtype=np.int32).tolist())
            elif req_len >= input_len:
                # 如果长度足够，截断到input_len
                padded_inputs.append(req[:input_len])
            else:
                # 如果长度不够，重复内容进行pad
                repeat_times = (input_len + req_len - 1) // req_len  # 向上取整
                padded_req = (req * repeat_times)[:input_len]
                padded_inputs.append(padded_req)
        input_ids = np.array(padded_inputs, dtype=np.int32)
    else:
...
def extend_chunked_prefill(reqs, model_runner, rank_print):
    ...
    while True:
        ...
        if logging_chuncked_prefill == "True":
            print(
                f"""rank {torch.distributed.get_rank()} {counter}: len(remaining_chunked_reqs): {len(remaining_chunked_reqs)}, remaining_chunked_reqs ids: {[req.rid for req in remaining_chunked_reqs.values()]}""")
    ...
    if len(accumulated_next_token_ids) == 0:
        ...
        for req_id in req_ids_ordered:
            if req_id in accumulated_next_token_ids:
                final_next_token_ids_list.append(
                    accumulated_next_token_ids[req_id])
                if req_id in accumulated_next_token_logits:
                    final_next_token_logits_list.append(
                        accumulated_next_token_logits[req_id])
        ...
        if len(final_next_token_ids_list) > 0:
            ...
            if len(final_next_token_logits_list) > 0:
                final_next_token_logits = torch.stack(
                    final_next_token_logits_list)
            else:
                final_next_token_logits = None
        else:
...
def correctness_test(
    ...
    custom_prompts = _read_prompts_from_file(
        bench_args.prompt_filename, rank_print)
    ...
    # Decode
    output_ids = [input_ids[i] + [next_token_ids[i]]
                  for i in range(len(input_ids))]
...
def profile_work_func(work_func, *args, **kwargs):
    torch.cuda.memory._record_memory_history(max_entries=100000)
    try:
        res = work_func(*args, **kwargs)
    finally:
        profile_dir = os.environ.get(
            "SGLANG_TORCH_PROFILER_DIR", "/tmp")
        torch.cuda.memory._dump_snapshot(
            f"""{profile_dir}/memory_snapshot_gpu{args[3]}.pkl""")
        ...
        try:
            ...
            if bench_args.profile:
                profile_dir = os.environ.get(
                    "SGLANG_TORCH_PROFILER_DIR", "/tmp")
                torch.cuda.memory._dump_snapshot(
                    f"""{profile_dir}/memory_snapshot.pkl""")
                torch.cuda.memory._record_memory_history(enabled=None)
    else:
        workers = []
