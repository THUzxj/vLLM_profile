...
def test_different_batch_sizes(batch_sizes, output_path):
    ...
    for j, stream0_size in enumerate(stream0_sizes):
        try:
            streams, resources = split_device_green_ctx(dev, num_group, stream0_size)
            print(
            ...
            for i, stream in enumerate(streams):
                with torch.cuda.stream(stream):
                    x = torch.randn(batch_sizes[i], 8192, 8192, device=dev, dtype=torch.bfloat16)
                    y = torch.randn(8192, 8192, device=dev, dtype=torch.bfloat16)
            ...
            for i in range(num_streams):
                ...
                print(f"Stream {i}, Resource: {resources[i].sm.smCount}, Elapsed time: {elapsed_time} ms")
    ...
    x = torch.randn(merged_batch_size, 8192, 8192, device=dev, dtype=torch.bfloat16)
    y = torch.randn(8192, 8192, device=dev, dtype=torch.bfloat16)
            ...
            try:
                ...
                for i, stream in enumerate(streams):
                    with torch.cuda.stream(stream):
                        ...
                        print(f"Stream {i}, Resource: {resources[i].sm.smCount}, Allocation time: {elapsed_time} ms")
                        start_time.record()
                        z = x @ y
...
if __name__ == "__main__":
    ...
    output_path = sys.argv[1] if len(sys.argv) > 1 else "green_ctx_batch_size_test.csv"
