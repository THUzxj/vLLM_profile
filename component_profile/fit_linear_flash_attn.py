...
def main():
    # 读取 CSV 文件
    csv_path = 'sweep_profile_result/benchmark_flash_attn_results.csv'
    ...
    print(f"Batch size * KV len 范围: [{batch_kv_product.min()}, {batch_kv_product.max()}]")
    ...
    # 保存图片
    output_path = 'sweep_profile_result/benchmark_flash_attn_linear_fit.png'
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"\n图片已保存到: {output_path}")
    
    # 保存拟合参数到文件
    params_path = 'sweep_profile_result/benchmark_flash_attn_linear_params.txt'
    with open(params_path, 'w') as f:
        f.write("线性函数拟合参数\n")
        f.write("=" * 50 + "\n\n")
        f.write(f"拟合公式:\n")
        f.write(f"  mean_time_ms = {slope:.10f} × (batch_size × kv_len) + {intercept:.10f}\n\n")
        f.write(f"参数:\n")
        ...
        f.write(f"  Batch size × KV len 范围: [{batch_kv_product.min()}, {batch_kv_product.max()}]\n")
        f.write(f"  Mean time 范围: [{mean_time_ms.min():.6f}, {mean_time_ms.max():.6f}] ms\n")
    ...
    plt.tight_layout()
    residual_path = 'sweep_profile_result/benchmark_flash_attn_linear_residuals.png'
