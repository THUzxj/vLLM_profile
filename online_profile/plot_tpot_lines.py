...
def main():
    if len(sys.argv) < 2:
        print("用法: python plot_tpot_lines.py <csv_file> [output_dir]")
        print("示例: python plot_tpot_lines.py bs_len_test_modelQwen3-4B_mpsno_summary.csv ./plots/")
        sys.exit(1)
    ...
    if 'input_tokens' in df.columns:
        print(f"输入 tokens 种类: {sorted(df['input_tokens'].unique())}")
    if 'batch_size' in df.columns:
        print(f"Batch sizes: {sorted(df['batch_size'].unique())}")

    # 保存所有图的两个版本
    print("生成图表1: TPOT vs Batch Size (linear & log)...")
    save_versions(plot_tpot_by_batch_size, df,
                  output_dir, 'tpot_vs_batch_size')

    print("生成图表2: TPOT vs Input Tokens (linear & log)...")
    save_versions(plot_tpot_by_input_tokens, df,
                  output_dir, 'tpot_vs_input_tokens')

    print("生成图表3: TPOT vs (batch_size × input_tokens) (linear & log)...")
    save_versions(plot_tpot_by_combined, df, output_dir, 'tpot_vs_combined')
